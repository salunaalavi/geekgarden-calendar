<template>
  ...
  >
    ...
    <input
      :class="{ 'warning': showValidation, 'opacity-50 cursor-not-allowed': disabled }"
      :type="type"
      :value="viewValue"
...
function emitNewVal(val) {
  ...
  const rawValue = props.parser && props.invertedParser ? props.invertedParser(parsedValue) : parsedValue;
...
function validateField(e) {
  ...
  if (props.validator) {
    ...
    showValidation = props.required && (rawValue === '' || rawValue === null || rawValue === undefined);
  }
